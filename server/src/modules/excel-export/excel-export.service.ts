...
export class ExcelExportService {
  ...
  async exportGiftLedger(params: {
    ...
    try {
      // 0. 付费校验：检查是否开通了礼账导出功能
      if (banquetId) {
        const exportEnabled = await this.paidFeaturesService.checkFeatureEnabled(banquetId, 'ledger_export')
        if (!exportEnabled) {
          throw new Error('该宴会未开通礼账导出功能，请先购买')
        }
      }
  ...
  async exportBanquetReport(params: {
    ...
    try {
      ...
      overviewSheet.addRow({ item: '宴会时间', content: new Date(banquet.event_time).toLocaleString('zh-CN') })
      ...
      overviewSheet.addRow({ item: '宴会状态', content: banquet.status === 'active' ? '进行中' : banquet.status === 'ended' ? '已结束' : '草稿' })
