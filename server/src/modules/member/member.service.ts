...
export class MemberService {
  ...
  async getMemberStatus(openid: string): Promise<MemberStatus> {
    ...
    if (error) {
      ...
      return {
        isMember: false,
        features: { aiWelcomePage: false, ledgerExport: false, customCover: false, giftReminder: false }
      }
    }
    ...
    const isMember = features.aiWelcomePage || features.ledgerExport || features.customCover || features.giftReminder
   ...
   * 创建购买订单
   */
  async createPurchaseOrder(openid: string, featureId: string, amount: number, mockPay: boolean = false): Promise<any> {
    // 检查是否已购买
