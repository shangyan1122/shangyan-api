...
// 推广等级佣金比例
// 宴客官(Lv.1): 商城7%, VIP18%
// 掌宴官(Lv.2): 商城12%, VIP23% — 有效用户≥10 + 成交额≥5000元
// 司宴官(Lv.3): 商城15%, VIP28% — 有效用户≥30 + 成交额≥2万元
export type PromoterLevel = 'yanke' | 'zhangyan' | 'siyan'

export const PROMOTER_LEVEL_RATES: Record<PromoterLevel, { mallRate: number; vipRate: number; minValidUsers: number; minAmount: number }> = {
  yanke:   { mallRate: 7,  vipRate: 18, minValidUsers: 0,  minAmount: 0 },
  zhangyan: { mallRate: 12, vipRate: 23, minValidUsers: 10, minAmount: 500000 },  // 5000元=500000分
  siyan:   { mallRate: 15, vipRate: 28, minValidUsers: 30, minAmount: 2000000 }   // 2万元=2000000分
}

const PROMOTER_LEVEL_ORDER: PromoterLevel[] = ['siyan', 'zhangyan', 'yanke'] // 从高到低
...
export class ReferralService {
  ...
  /**
   * 根据用户的直推有效用户数和成交额，判定推广等级
   * 有效用户 = 成功举办过宴会的直推下级
   */
  async getPromoterLevel(userId: string): Promise<PromoterLevel> {
    try {
      const now = new Date()
      ...
      const { data: binds } = await this.supabase
        .from('referral_binds')
        .select('invitee_id, total_commission, commission_expire_date')
        .eq('referrer_id', userId)

      if (!binds || binds.length === 0) return 'yanke'

      // 筛选有效绑定（未过期）
      const activeBinds = binds.filter((b: any) => new Date(b.commission_expire_date) >= now)
      ...
      for (const bind of activeBinds) {
        // 检查该下级是否举办过宴会
        const { data: hostedBanquets } = await this.supabase
          .from('banquets')
          .select('id')
          .eq('host_openid', bind.invitee_id)
          .limit(1)

        if (hostedBanquets && hostedBanquets.length > 0) {
          validUserCount++
        }

        totalAmount += (bind as any).total_commission || 0
      }

      // 从高到低匹配等级
      for (const level of PROMOTER_LEVEL_ORDER) {
        const config = PROMOTER_LEVEL_RATES[level]
        if (validUserCount >= config.minValidUsers && totalAmount >= config.minAmount) {
          return level
        }
      }
      ...
      this.logger.error('判定推广等级失败:', error)
      return 'yanke'
    }
  }
   ...
   * 根据推广等级获取佣金比例
   */
  getCommissionRates(level: PromoterLevel): { mallRate: number; vipRate: number } {
    const config = PROMOTER_LEVEL_RATES[level]
    return { mallRate: config.mallRate, vipRate: config.vipRate }
  }
   ...
   * 获取分销统计数据
   */
  async getReferralStats(openid: string): Promise<{ code: number; msg: string; data: ReferralStats }> {
    try {
      // 获取用户ID和上级信息
      const { data: user } = await this.supabase
        .from('users')
      ...
      if (!user) {
        return {
          ...
          data: {
            ...
            pendingCommission: 0,
            vipInvitees: 0,
            expireDate: new Date(Date.now() + COMMISSION_BIND_YEARS * 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
            isFreePerson: true,
      ...
      const { data: binds, error: bindsError } = await this.supabase
        ...
        .select(`
          ...
          users!referral_binds_invitee_id_fkey (id, is_vip, vip_expire_date)
        `)
      ...
      const vipInvitees = activeBinds.filter((b: any) => b.users?.is_vip).length
      ...
      const availableCommission = commissions?.filter((c: any) => c.status === 'available')
        .reduce((sum: number, c: any) => sum + (c.amount || 0), 0) || 0
      const pendingCommission = commissions?.filter((c: any) => c.status === 'pending')
        .reduce((sum: number, c: any) => sum + (c.amount || 0), 0) || 0
  ...
  async getInvitees(openid: string): Promise<{ code: number; msg: string; data: Invitee[] }> {
    try {
      ...
      const { data: inviteeUsers } = await this.supabase
        .from('users')
        .select('id, nickname, avatar, is_vip, vip_expire_date')
        .in('id', inviteeIds)
      ...
      for (const bind of binds) {
        ...
        const totalSpent = giftRecords?.reduce((sum: number, g: any) => sum + (g.amount || 0), 0) || 0
        ...
        invitees.push({
          ...
          isVip: inviteeUser.is_vip || false,
          vipExpireDate: inviteeUser.vip_expire_date,
          totalSpent,
   ...
   * 获取或创建邀请码
   */
  async getOrCreateReferralCode(openid: string): Promise<{ code: number; msg: string; data: { code: string } }> {
    try {
      // 获取用户ID
  ...
  async bindOnGift(
    ...
    try {
      ...
      if (existingBind) {
        ...
        const { error: bindError } = await this.supabase
          ...
          .insert({
            ...
            commission_rate: PROMOTER_LEVEL_RATES.yanke.mallRate,  // 初始绑定用宴客官比例，实际计算时按等级动态获取
            commission_expire_date: expireDate.toISOString(),
      ...
      await this.supabase
        .from('users')
        .update({ referrer_id: hostUser.id })
        .eq('id', guestUser.id)
  ...
  /**
   * 获取佣金分账参数（供分账服务调用，不写数据库）
   * 佣金通过微信分账实时到账，无需提现
   * 
   * @param guestOpenid 支付用户openid
   * @param amount 支付金额（分）
   * @param type 消费类型
   * @returns 佣金分账参数，无佣金时返回null
   */
  async getCommissionSharingParams(
    guestOpenid: string,
    amount: number,
    type: 'vip' | 'mall' | 'gift' = 'gift'
  ): Promise<{
    commissionAmount: number
    commissionOpenid: string
    commissionRate: number
    promoterLevel: string
    isPlatformCommission: boolean
  } | null> {
    try {
      // 获取用户信息
      const { data: user } = await this.supabase
      ...
      if (!user) {
        this.logger.log(`用户 ${guestOpenid} 不存在，无佣金分账`)
        return null
      }
      ...
      if (user.officer_id) {
        ...
        if (officerBind && new Date(officerBind.commission_expire_date) >= new Date()) {
          if (type === 'vip' || type === 'mall') {
            // 获取推荐官信息
            const { data: officer } = await this.supabase
              .from('recommend_officers')
              .select('openid, vip_commission_rate, mall_commission_rate')
            ...
            if (officer) {
              ...
              if (commissionAmount > 0) {
                return {
                  commissionAmount,
                  commissionOpenid: officer.openid,
                  commissionRate: rate,
                  promoterLevel: 'recommend_officer',
      ...
      if (isFree) {
        // 自由人消费，佣金归平台（不需要分给个人，留在商户号即可）
        this.logger.log(`自由人 ${guestOpenid} 消费，佣金归平台`)
        return null
      }
      ...
      // 获取上级的推广等级
      const promoterLevel = await this.getPromoterLevel(user.referrer_id)
      const rates = this.getCommissionRates(promoterLevel)
      const commissionRate = type === 'vip' ? rates.vipRate : rates.mallRate
      ...
      if (!referrer?.openid) {
        this.logger.error(`上级用户 ${user.referrer_id} openid不存在`)
        return null
      }

      return {
        commissionAmount,
        commissionOpenid: referrer.openid,
        commissionRate,
        promoterLevel,
        isPlatformCommission: false
      }
    } catch (error) {
      this.logger.error('获取佣金分账参数失败:', error)
      return null
    }
  }

  /**
   * 计算并记录佣金（仅用于非分账场景的兼容，新场景使用分账实时到账）
   * @deprecated 新场景请使用 getCommissionSharingParams + 分账
   */
  async calculateCommission(
    ...
    try {
      ...
      if (user.officer_id) {
        ...
        if (officerBind && new Date(officerBind.commission_expire_date) >= new Date()) {
          // 有有效的推荐官绑定，调用推荐官佣金记录
          // 只有 VIP 和商城消费才记录推荐官佣金，礼品兑换不记录
          if (type === 'vip' || type === 'mall') {
            const { RecommendOfficerService } = await import('../recommend-officer/recommend-officer.service')
            const officerService = new RecommendOfficerService()

            const result = await officerService.recordCommission(
              user.officer_id || '',
              user.id,
              type,
              amount,
            ...
            if (result.code === 200 && result.msg !== 'success') {
              this.logger.error(`推荐官佣金记录失败: ${result.msg}`)
            } else if (result.code === 200) {
              // 计算佣金金额
              const officerResult = await this.supabase
                .from('recommend_officers')
                .select('vip_commission_rate, mall_commission_rate')
                .eq('id', user.officer_id)
                .single()

              if (officerResult.data) {
                const rate = type === 'vip' ? officerResult.data.vip_commission_rate : officerResult.data.mall_commission_rate
                const commissionAmount = Math.floor(amount * rate / 100)
                return { success: true, commissionAmount, referrerId: user.officer_id }
              }
            }
          }
          // 推荐官佣金记录失败，继续走原有逻辑
      ...
      let promoterLevel: PromoterLevel = 'yanke'

      if (!isFree && user.referrer_id) {
        // 获取上级的推广等级
        promoterLevel = await this.getPromoterLevel(user.referrer_id)
      }

      const rates = this.getCommissionRates(promoterLevel)
      commissionRate = type === 'vip' ? rates.vipRate : rates.mallRate
      ...
      return { success: true, commissionAmount, referrerId: isPlatformCommission ? PLATFORM_ACCOUNT_ID : commissionRecipientId }
    } catch (error) {
      ...
      if (existingBind) {
        ...
        await this.supabase
          ...
          .insert({
            ...
            commission_rate: PROMOTER_LEVEL_RATES.yanke.mallRate,  // 初始绑定用宴客官比例
            commission_expire_date: expireDate.toISOString(),
            created_at: new Date().toISOString()
          })
      }
