...
export class WithdrawService {
   ...
   * 
   * 随礼金通过微信支付V3分账直达主办方，无需提现
   * 此接口保留用于对账查询，实际到账金额 = 随礼金额 × 99.4%
   */
  async getAvailableBalance(openid: string): Promise<number> {
    ...
    // 4. 计算可提现金额
    // 随礼金通过分账已直达主办方（99.4%），此处为对账计算
    // 综合服务费0.6%（含微信支付通道费0.56%+平台服务费0.04%），主办方到账99.4%
   ...
   * 申请提现
   */
  async applyWithdraw(openid: string, amount: number): Promise<{
    success: boolean
    withdrawId?: string
    errorMsg?: string
  }> {
