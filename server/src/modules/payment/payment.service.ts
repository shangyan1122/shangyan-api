 ...
 * 4. 分账失败 → 自动重试3次，通知管理员
 * 
 * 【分账规则 - 固定0.6%综合服务费，主办方到账99.4%】
 * 优惠期（微信费率0.56%）：
 *   微信手续费0.56% + 平台收入0.04% + 主办方到账99.40% = 100%
 * 优惠失效后（微信费率0.60%）：
 *   微信手续费0.60% + 平台收入0.00% + 主办方到账99.40% = 100%
 * 对外统一：手续费0.6%，到账99.4%
 * 
 * 【关键特性】
 * - 使用微信官方分账API，禁止平台转账、禁止二清
 * - 分账接收方为主办方微信OPENID，金额为订单的99.4%
 * - 主办方无需营业执照，仅需微信授权登录
 * - 分账失败自动重试3次并通知管理员
 * - 回礼资金完全独立，从充值余额发放
 */
...
export class PaymentService {
  ...
  // 支付回调URL（支持多种环境变量名称）
  private readonly notifyUrl = process.env.WECHAT_PAY_NOTIFY_URL || process.env.WECHAT_NOTIFY_URL || ''
  ...
  private loadMerchantCert() {
    const pfxPath = process.env.WECHAT_PFX_PATH || join(process.cwd(), 'certs', 'apiclient_cert.p12')
   ...
   * 生成模拟支付结果（开发测试用）
   */
  private async createMockPayment(orderId: string, params: WechatPaymentParams): Promise<PaymentResult> {
    this.logger.log(`使用模拟支付: ${orderId}`)

    const timeStamp = Math.floor(Date.now() / 1000).toString()
    const nonceStr = this.generateNonceStr()

    // 创建待支付的随礼记录（使用 upsert 避免重复创建报错）
  ...
  async handlePaymentCallback(body: any): Promise<{
    ...
    try {
      ...
      // 4.1 幂等性检查：如果已支付且transaction_id已记录，跳过重复回调
      if (giftRecord.payment_status === 'paid' && giftRecord.transaction_id === transaction_id) {
        this.logger.log(`支付回调已处理(幂等): orderId=${out_trade_no}, transactionId=${transaction_id}`)
      ...
      // 6. 【核心】调用微信支付V3分账API（礼金分账：主办方99.4% + 平台0.6%，上级不分）
      ...
      if (hostOpenid && giftRecord.amount > 0 && transaction_id) {
        // 绑定上下级关系（嘉宾→主办方，3年有效期）
        try {
          const { ReferralService } = await import('../referral/referral.service')
          const referralService = new ReferralService()
          await referralService.bindOnGift(giftRecord.guest_openid, hostOpenid, out_trade_no)
        } catch (err: any) {
          this.logger.warn(`绑定上下级失败（不影响分账）: ${err.message}`)
        }
  ...
  private async executeProfitSharingWithRetry(
    ...
    if (!this.profitSharingService) {
      ...
      await supabase
        ...
        .eq('id', orderId)
      return { success: true, outOrderNo: `MOCK_PS_${Date.now()}` }
    }

    const result = await this.profitSharingService.executeGiftProfitSharingWithRetry(
      transactionId, hostOpenid, amount, orderId, 3
    )
