...
export class AdminOrderService {
  ...
  async getOrders(params: {
    ...
    if (banquetIds.length > 0) {
      const { data: banquets } = await client
        ...
        .select('id, name')
        .in('id', banquetIds)
      banquets?.forEach(b => { banquetMap[b.id] = b })
    }
  ...
  async refundOrder(orderId: string): Promise<{ code: number; msg: string; data: null }> {
    ...
    if (order.status !== 'paid') {
      return { code: 400, msg: '订单状态不正确，无法退款', data: null }
    }
    ...
    try {
      ...
      const refundResult = await this.wechatPayService.refund({
        ...
        refundId: refundId,
        totalAmount: order.pay_amount || order.total_amount,
        refundAmount: order.pay_amount || order.total_amount,
        reason: '管理员退款'
      })

      if (!refundResult.success) {
        this.logger.error(`微信支付退款失败: ${refundResult.message}`)
        return { code: 500, msg: refundResult.message || '退款失败', data: null }
      }
      ...
      const { error: updateError } = await client
        ...
        .update({
          ...
          refund_id: refundId,
          refund_status: refundResult.data?.refund_status || 'PROCESSING',
          refund_amount: refundResult.data?.amount?.refund || order.pay_amount
        })
        .eq('id', orderId)

      if (updateError) {
        this.logger.error(`订单退款状态更新失败: ${updateError.message}`)
        return { code: 500, msg: '退款状态更新失败', data: null }
      }

      this.logger.log(`订单退款成功: orderId=${orderId}, refundId=${refundId}`)
      return { code: 200, msg: '退款成功', data: null }
    } catch (error: any) {
      this.logger.error(`订单退款异常: ${error.message}`)
   ...
   * 批量发货
   */
  async batchShipOrders(orderIds: string[]): Promise<{ code: number; msg: string; data: { success: string[]; failed: { id: string; reason: string }[] } }> {
    const client = getSupabaseClient()
    ...
    for (const orderId of orderIds) {
      try {
        ...
        failed.push({ id: orderId, reason: '未知错误' })
      }
    }

    this.logger.log(`批量发货完成: 成功 ${success.length} 个, 失败 ${failed.length} 个`)
    return {
      code: 200,
      msg: `批量发货完成: 成功 ${success.length} 个, 失败 ${failed.length} 个`,
      data: { success, failed }
    }
  }
   ...
   * 批量取消订单
   */
  async batchCancelOrders(orderIds: string[], reason?: string): Promise<{ code: number; msg: string; data: { success: string[]; failed: { id: string; reason: string }[] } }> {
    const client = getSupabaseClient()
    const success: string[] = []
    const failed: { id: string; reason: string }[] = []
    ...
    this.logger.log(`批量取消订单完成: 成功 ${success.length} 个, 失败 ${failed.length} 个`)
    return {
      code: 200,
      msg: `批量取消订单完成: 成功 ${success.length} 个, 失败 ${failed.length} 个`,
      data: { success, failed }
    }
  }
   ...
   * 批量导出订单
   */
  async batchExportOrders(orderIds: string[]): Promise<{ code: number; msg: string; data: { downloadUrl?: string } }> {
    // 获取订单详情
    const client = getSupabaseClient()
    const { data: orders, error } = await client
      .from('mall_orders')
      .select('*, users(*), mall_order_items(*)')
      .in('id', orderIds)

    if (error || !orders) {
      return { code: 500, msg: '获取订单数据失败', data: {} }
    }

    // TODO: 生成 Excel 文件并上传到对象存储
    // 这里简化处理，返回模拟数据

    this.logger.log(`批量导出订单: ${orders.length} 个订单`)
    return {
      code: 200,
      msg: '订单导出成功',
      data: {
        downloadUrl: '/api/download/orders.xlsx'
      }
    }
  }
}
