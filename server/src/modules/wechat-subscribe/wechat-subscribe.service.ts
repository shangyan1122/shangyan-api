...
const SUBSCRIBE_TEMPLATE_IDS = {
  ...
  // 人情绑定对方办宴即时通知模板
  BANQUET_CREATED_NOTIFY: process.env.WECHAT_TEMPLATE_BANQUET_CREATED_NOTIFY || '',
...
export class WechatSubscribeService {
   ...
   * 发送订阅消息
   */
  async sendSubscribeMessage(message: SubscribeMessage): Promise<{ success: boolean; errcode?: number; errmsg?: string }> {
   ...
   * - time3: 宴会时间
   * - thing4: 宴会地点
   * - thing5: 提示内容（如"您的好友XX正在举办宴会"）
   */
  async sendBanquetCreatedNotify(params: {
    openid: string           // 被通知的对方
    hostName: string         // 办宴人姓名
    banquetType: string      // 宴会类型
    banquetName: string      // 宴会名称
    eventTime: string        // 宴会时间
    location: string         // 宴会地点
  }): Promise<{ success: boolean }> {
    const { openid, hostName, banquetType, banquetName, eventTime, location } = params
    
    const templateId = SUBSCRIBE_TEMPLATE_IDS.BANQUET_CREATED_NOTIFY
    
    if (!templateId) {
      this.logger.warn('宴会创建通知模板ID未配置，使用宴会提醒模板替代')
      // 降级使用宴会提醒模板
      return this.sendBanquetReminder({
        openid,
        banquetName,
        banquetType,
        eventTime,
    ...
    const result = await this.sendSubscribeMessage({
      ...
      page: 'pages/index/index',
      data: {
        thing1: { value: banquetType.substring(0, 20) },
        thing2: { value: banquetName.substring(0, 20) },
        time3: { value: eventTime },
        thing4: { value: location.substring(0, 20) },
        thing5: { value: `${hostName}正在举办宴会，记得礼尚往来`.substring(0, 20) }
      }
    })
   ...
   * 微信限制：每个用户每月接收订阅消息上限
   */
  async batchSendSubscribeMessages(messages: SubscribeMessage[]): Promise<{ success: number; failed: number }> {
    let success = 0
    let failed = 0
