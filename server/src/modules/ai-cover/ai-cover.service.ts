...
/**
 * AI 封面生成服务 - 使用硅基流动 (SiliconFlow) 图片生成 API
 */
@Injectable()
export class AICoverService {
  ...
  private readonly imageModel = process.env.SILICONFLOW_IMAGE_MODEL || 'Kwai-Kolors/Kolors'
  ...
  async generateBanquetCover(params: {
    ...
    const prompt = customPrompt || this.buildPrompt(banquetType, banquetName, style)
