...
describe('ReturnGiftService', () => {
  ...
  describe('saveReturnGiftSettings', () => {
    ...
    it('创建失败时应该抛出错误', async () => {
      ...
      await expect(service.saveReturnGiftSettings('banquet-1', settings))
  ...
  describe('triggerReturnGift', () => {
    it('随礼记录不存在时应该抛出错误', async () => {
      ...
      await expect(service.triggerReturnGift('non-existent'))
        .rejects.toThrow('随礼记录不存在')
    })
  })
  ...
  describe('claimOnsiteGift', () => {
    it('已领取时应该抛出错误', async () => {
      ...
      await expect(service.claimOnsiteGift('return-unique-1'))
        .rejects.toThrow('现场礼品已领取')
    })
    ...
    it('回礼记录不存在时应该抛出错误', async () => {
      ...
      await expect(service.claimOnsiteGift('non-existent'))
        .rejects.toThrow('回礼记录不存在')
    })
  })
  ...
  describe('claimMallGift', () => {
    it('已领取时应该抛出错误', async () => {
      ...
      await expect(service.claimMallGift('return-mall-unique-1', 'delivery'))
        .rejects.toThrow('商城礼品已领取')
    })
    ...
    it('回礼记录不存在时应该抛出错误', async () => {
      ...
      await expect(service.claimMallGift('non-existent', 'delivery'))
        .rejects.toThrow('回礼记录不存在')
    })
  })
  ...
  describe('exchangeOnsiteGift', () => {
    ...
    it('已核销时应该抛出错误', async () => {
      ...
      await expect(service.exchangeOnsiteGift('CODE123'))
        .rejects.toThrow('礼品已核销')
    })
    ...
    it('兑换码无效时应该抛出错误', async () => {
      ...
      await expect(service.exchangeOnsiteGift('NONEXISTENT'))
        .rejects.toThrow('兑换码无效')
    })
    ...
    it('兑换码过期时应该抛出错误', async () => {
      ...
      await expect(service.exchangeOnsiteGift('EXPIRED'))
        .rejects.toThrow('兑换码已过期')
    })
  })
