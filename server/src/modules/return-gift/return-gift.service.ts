...
export class ReturnGiftService {
  ...
  async claimMallGift(
    ...
    // 5. 检查库存
    const availableGift = settings.mall_gift_items.find(
      item => item.remaining_count > 0
    )
  ...
  async claimOnsiteGift(returnGiftId: string) {
    ...
    // 5. 检查库存
    const availableGift = settings.onsite_gift_items.find(
      item => item.remaining_count > 0
    )
   ...
   * 只有主办方指定的工作人员微信号才能核销
   */
  async exchangeOnsiteGiftWithAuth(exchangeCode: string, verifierWechat: string, verifierOpenid?: string) {
  ...
  private async updateReturnGiftStatus(returnGiftId: string) {
    ...
    if (mallGiftDone && onsiteGiftDone) {
      status = 'completed'
  ...
  async refundUnclaimedGifts(banquetId: string) {
    ...
    // 8. 【核心】退款完成后，按实际消费金额一次性分账
    //    实际消费 = 支付总金额 - 退款金额
    //    分给上级佣金（如果有），剩余归平台
    await this.triggerMallProfitSharingAfterRefund(banquet, refundAmount)
  ...
  async updateDeliveryInfo(
    ...
    try {
      // 1. 验证记录存在（查询 guest_return_gifts 表）
      ...
      // 2. 更新收货信息（更新 guest_return_gifts 表）
   ...
   * - finish=true 一次性完结
   */
  private async triggerMallProfitSharingAfterRefund(
    banquet: { host_openid: string; name: string; mall_gift_pay_no?: string; mall_gift_pay_amount?: number },
    refundAmount: number
  ): Promise<void> {
    try {
      // 如果没有微信交易号，跳过分账（可能是模拟支付）
      if (!banquet.mall_gift_pay_no) {
        this.logger.log(`宴会无微信支付单号，跳过商城分账`)
        return
      }

      const totalPaid = banquet.mall_gift_pay_amount || 0
      const actualAmount = totalPaid - refundAmount  // 实际消费金额

      if (actualAmount <= 0) {
        this.logger.log(`实际消费金额为0，跳过商城分账`)
        return
      }

      this.logger.log(`商城分账: 总支付=${totalPaid}分, 退款=${refundAmount}分, 实际消费=${actualAmount}分`)
      ...
      if (!commissionParams) {
        this.logger.log(`无佣金参数，跳过分账`)
        return
      }

      // 查询微信支付交易号（transaction_id）
      // mall_gift_pay_no 是 out_trade_no，需要查 transaction_id
      const { data: orderData } = await supabase
        .from('mall_orders')
        .select('pay_transaction_id')
        .eq('order_no', banquet.mall_gift_pay_no)
        .single()

      const transactionId = orderData?.pay_transaction_id
      if (!transactionId) {
        this.logger.warn(`找不到微信交易号，跳过商城分账: orderNo=${banquet.mall_gift_pay_no}`)
        return
      }
      ...
      const result = await profitSharingService.executeMallProfitSharingWithRetry(
        transactionId,
        banquet.mall_gift_pay_no!,
        actualAmount,
        commissionParams,
        3
      )
      ...
      if (result.success) {
        this.logger.log(`商城分账成功: 佣金=${result.commissionAmount || 0}分, 平台=${result.platformAmount || 0}分`)
      } else {
        this.logger.error(`商城分账失败: ${result.errorMsg}`)
      }
    } catch (error: any) {
      this.logger.error(`商城分账异常: ${error.message}`)
    }
  }
}
