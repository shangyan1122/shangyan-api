...
export class ShoppingVoucherService {
   ...
   */
  async useVoucher(openid: string, amount: number, orderId?: string): Promise<{ success: boolean; usedAmount: number; remainingBalance: number }> {
   ...
   * 手动发放购物券（管理员操作）
   */
  async issueVoucherManually(openid: string, amount: number, description: string): Promise<ShoppingVoucher | null> {
    const voucherCode = this.generateVoucherCode()
