...
export class AdminProductService {
   ...
   * 调整商品价格和服务费
   */
  async updateProductPrice(id: string, params: {
    price: number
   ...
   * 目前返回模拟数据供前端测试
   */
  async search1688Products(keyword: string, page: number = 1, pageSize: number = 20): Promise<{
    ...
    const mockProducts: Alibaba1688Product[] = [
      ...
      },
      {
        ...
        images: [
          'https://cbu01.alicdn.com/img/example3.jpg'
        ],
        price: 200,  // 2元
        category: '红包',
      ...
      },
      {
        ...
        images: [
          'https://cbu01.alicdn.com/img/example5.jpg'
        ],
        price: 3200,  // 32元
        category: '生活用品',
  ...
  async importFrom1688(params: {
    ...
    const product = await this.createProduct({
      ...
      gift_type: params.giftType || 'delivery',
   ...
   * 调整库存
   */
  async updateStock(id: string, body: { quantity: number; reason?: string }): Promise<{ productId: string; oldStock: number; newStock: number }> {
    // 获取当前商品信息
    ...
    if (updateError) {
      this.logger.error('更新库存失败:', updateError)
    ...
    if (logError) {
      this.logger.error('记录库存日志失败:', logError)
    }

    this.logger.log(`库存调整成功: productId=${id}, oldStock=${oldStock}, newStock=${newStock}`)
    return { productId: id, oldStock, newStock }
  }

  /**
   * 批量调整库存
   */
  async batchUpdateStock(items: { productId: string; quantity: number; reason?: string }[]): Promise<{
    success: string[]
    ...
    for (const item of items) {
      try {
        ...
        success.push(item.productId)
      } catch (err: any) {
        failed.push({ productId: item.productId, reason: err.message })
      }
    }
   ...
   * 获取库存日志
   */
  async getStockLogs(productId: string, params: { page: number; pageSize: number }): Promise<{
    logs: any[]
    total: number
  }> {
    ...
    if (error) {
      this.logger.error('获取库存日志失败:', error)
      throw new Error('获取库存日志失败')
    }
  ...
  async getLowStockProducts(params: {
    ...
    const { threshold, page, pageSize } = params
    const offset = (page - 1) * pageSize
    ...
    if (error) {
      this.logger.error('获取低库存列表失败:', error)
      throw new Error('获取低库存列表失败')
    }
