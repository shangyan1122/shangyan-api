...
export class AdminProductController {
  ...
  @Post('import-1688')
  @RequirePermissions('product:write')
  async importFrom1688(@Body() body: {
    productId: string
    category: string
    stock?: number
    markupRate?: number
    giftType?: 'onsite' | 'delivery'
  }) {
  ...
  @RequirePermissions('product:write')
  async batchImportFrom1688(@Body() body: {
    items: {
      productId: string
      category: string
      stock?: number
  ...
  async updateProduct(
    @Param('id') id: string,
    @Body() body: Partial<CreateProductParams>
  ) {
  ...
  async updateStock(
    @Param('id') id: string,
    @Body() body: {
      quantity: number  // 变动数量（正数增加，负数减少）
      reason?: string   // 调整原因
    }
  ) {
    this.logger.log(`调整库存: id=${id}, quantity=${body.quantity}`)

    try {
      const result = await this.adminProductService.updateStock(id, body)
      return { code: 200, msg: '库存调整成功', data: result }
    } catch (error: any) {
      this.logger.error(`调整库存失败: ${error.message}`)
      return { code: 500, msg: error.message || '调整库存失败', data: null }
    }
  }
  ...
  @Post('batch-stock')
  @RequirePermissions('product:write')
  async batchUpdateStock(@Body() body: {
    items: {
      productId: string
      quantity: number
      reason?: string
    ...
    this.logger.log(`批量调整库存: ${body.items.length}个商品`)

    try {
      const result = await this.adminProductService.batchUpdateStock(body.items)
      return {
        code: 200,
        msg: `批量调整完成: 成功${result.success}个, 失败${result.failed}个`,
        data: result
      }
    } catch (error: any) {
      this.logger.error(`批量调整库存失败: ${error.message}`)
      return { code: 500, msg: '批量调整库存失败', data: null }
    }
  }
  ...
  async getStockLogs(
    ...
    this.logger.log(`获取库存日志: id=${id}`)

    try {
      const result = await this.adminProductService.getStockLogs(id, {
        page: page ? parseInt(page) : 1,
        pageSize: pageSize ? parseInt(pageSize) : 20
      })
      return {
      ...
      this.logger.error(`获取库存日志失败: ${error.message}`)
      return { code: 500, msg: '获取库存日志失败', data: [], total: 0 }
    }
  }
  ...
  async getLowStockProducts(
    ...
    this.logger.log(`获取低库存预警列表`)

    try {
      const result = await this.adminProductService.getLowStockProducts({
      ...
      this.logger.error(`获取低库存预警列表失败: ${error.message}`)
      return { code: 500, msg: '获取低库存预警列表失败', data: [], total: 0 }
    }
  }
}
