...
export class GiftExchangeService {
   ...
   * 差价支付成功回调
   */
  async handleDiffPaymentSuccess(
    exchangeNo: string,
  ...
  async getUserAvailableGifts(userOpenid: string): Promise<any[]> {
    ...
    if (!userError && userGifts) {
      results.push(...userGifts.map((item: any) => ({
        id: item.id,
    ...
    if (!guestError && guestGifts) {
      results.push(...guestGifts.map((item: any) => ({
        id: item.id,
  ...
  async previewExchange(
    ...
    const sourceTotalValue = sourceItems.reduce(
      (sum, item) => sum + item.price * item.quantity,
      0
    )

    const targetTotalValue = targetItems.reduce(
      (sum, item) => sum + item.price * item.quantity,
      0
    )
