import { pgTable, serial, timestamp, varchar, integer, text, boolean, jsonb, index, numeric } from "drizzle-orm/pg-core"
...
export const giftProducts = pgTable(
  "gift_products",
  {
    ...
    alibaba1688SyncedAt: timestamp("alibaba_1688_synced_at", { withTimezone: true, mode: 'string' }),
    source: varchar("source", { length: 20 }).default("manual"), // manual/alibaba_1688
    exchangeServiceFeeRate: numeric("exchange_service_fee_rate", { precision: 5, scale: 4 }).default("0.0500"), // 置换服务费率，默认5%
    createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' })
      .defaultNow()
      .notNull(),
    updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }),
...
export const referralBinds = pgTable(
  "referral_binds",
  {
    ...
    commissionExpireDate: timestamp("commission_expire_date", { withTimezone: true, mode: 'string' }).notNull(),
...
export const recommendOfficers = pgTable(
  "recommend_officers",
  {
    ...
    vipCommissionRate: integer("vip_commission_rate").default(18).notNull(), // VIP佣金比例，默认18%（宴客官等级）
    mallCommissionRate: integer("mall_commission_rate").default(7).notNull(), // 商城佣金比例，默认7%（宴客官等级）
    totalCommission: integer("total_commission").default(0).notNull(), // 累计佣金（分）
    availableCommission: integer("available_commission").default(0).notNull(), // 可提现佣金（分）
